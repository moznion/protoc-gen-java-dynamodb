...
import net.moznion.protoc.plugin.dynamodb.generated.AllTypesEntityProto;
...
public class AllTypesEntityTest {
	...
	void shouldInsertAndLookupEntitySuccessfully() throws Exception {
		final CreateTableRequest createTableRequest = dynamodbMapper.generateCreateTableRequest(
			AllTypesEntityProto.AllTypesEntity.DynamoDBEntity.class
		...
		final AllTypesEntityProto.AllTypesEntity fooEntity =
			AllTypesEntityProto.AllTypesEntity.newBuilder()
											  .setHashKeyStr(fooHashKey)
											  .build();
		dynamodbMapper.save(fooEntity.toDynamoDBEntity());

		final String barHashKey = "bar-hash-key";
		final AllTypesEntityProto.AllTypesEntity barEntity =
			AllTypesEntityProto.AllTypesEntity.newBuilder()
		...
		{
			final PaginatedQueryList<AllTypesEntityProto.AllTypesEntity.DynamoDBEntity> retrieved =
				dynamodbMapper.query(
					AllTypesEntityProto.AllTypesEntity.DynamoDBEntity.class,
					new DynamoDBQueryExpression<AllTypesEntityProto.AllTypesEntity.DynamoDBEntity>().withHashKeyValues(
						fooEntity.toDynamoDBEntity())
				);
