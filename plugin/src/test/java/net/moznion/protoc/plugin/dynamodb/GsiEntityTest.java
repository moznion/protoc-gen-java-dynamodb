...
public class GsiEntityTest {
	...
	void shouldInsertAndLookupEntitySuccessfully() throws Exception {
		final CreateTableRequest createTableRequest = dynamodbMapper
		...
		createTableRequest.setGlobalSecondaryIndexes(Arrays.asList(
			new GlobalSecondaryIndex().withIndexName("gsi0")
									  .withProvisionedThroughput(DEFAULT_TABLE_THROUGHPUT)
									  .withKeySchema(
										  new KeySchemaElement().withAttributeName("hashKeyStr")
																.withKeyType(KeyType.HASH),
										  new KeySchemaElement().withAttributeName("barStr")
																.withKeyType(KeyType.RANGE)
									  )
									  .withProjection(
										  new Projection().withProjectionType(ProjectionType.ALL)),
			new GlobalSecondaryIndex().withIndexName("gsi1")
									  .withProvisionedThroughput(DEFAULT_TABLE_THROUGHPUT)
									  .withKeySchema(
										  new KeySchemaElement().withAttributeName("fooStr")
																.withKeyType(KeyType.HASH),
										  new KeySchemaElement().withAttributeName("barStr")
																.withKeyType(KeyType.RANGE)
									  )
									  .withProjection(
										  new Projection().withProjectionType(ProjectionType.ALL)),
			new GlobalSecondaryIndex().withIndexName("gsi2")
									  .withProvisionedThroughput(DEFAULT_TABLE_THROUGHPUT)
									  .withKeySchema(
										  new KeySchemaElement().withAttributeName("fooStr")
																.withKeyType(KeyType.HASH),
										  new KeySchemaElement().withAttributeName("buzStr")
																.withKeyType(KeyType.RANGE)
									  )
									  .withProjection(
										  new Projection().withProjectionType(ProjectionType.ALL)),
			new GlobalSecondaryIndex().withIndexName("gsi3")
									  .withProvisionedThroughput(DEFAULT_TABLE_THROUGHPUT)
									  .withKeySchema(
										  new KeySchemaElement().withAttributeName("quxStr")
																.withKeyType(KeyType.HASH)
									  )
									  .withProjection(
										  new Projection().withProjectionType(ProjectionType.ALL))
		...
		final GsiEntityProto.GsiEntity entity = GsiEntityProto.GsiEntity.newBuilder()
					...
					new DynamoDBQueryExpression<GsiEntityProto.GsiEntity.DynamoDBEntity>()
						...
						.withRangeKeyCondition(
							...
							new Condition().withComparisonOperator(ComparisonOperator.EQ)
										   .withAttributeValueList(Collections.singletonList(
											   new AttributeValue().withS(bar)))
						)
				);
