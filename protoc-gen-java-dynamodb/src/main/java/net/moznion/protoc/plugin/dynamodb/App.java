...
public class App extends Generator {
  ...
  @Override
  public List<File> generateFiles(CodeGeneratorRequest originalRequest) throws GeneratorException {
            ...
            file -> {
              if (request.getFileToGenerateList().contains(file.getName())) {
                ...
                if (opts.shouldGenerate()) {
                  return Optional.of((Supplier<Stream<File>>) () -> handleProtoFile(file, opts));
                }
              }
  ...
  private Stream<File> handleProtoFile(
      final FileDescriptorProto fileDescriptor, final FileOptions opts) {
    ...
    return fileDescriptor.getMessageTypeList().stream()
        .flatMap(
            descriptor ->
                codeGenerator.generateDynamodbEntityCode(
                    descriptor,
